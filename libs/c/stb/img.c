...
static int l_encode_paletted_png(lua_State *L)
{
  ...
  if (!lua_isstring(L, 1))
  {
    err = ERR_ENCPLT_PAR1NOSTR;
    goto cleanup;
  }
  if (!lua_istable(L, 2))
  {
    err = ERR_ENCPLT_PAR2NOTBL;
    goto cleanup;
  }
  if (!lua_istable(L, 3))
  {
    err = ERR_ENCPLT_PAR3NOTBL;
    goto cleanup;
  }
  if (!lua_isinteger(L, 4))
  {
    err = ERR_ENCPLT_PAR4NONUM;
    goto cleanup;
  }
  if (!lua_isinteger(L, 5))
  {
    err = ERR_ENCPLT_PAR5NONUM;
    goto cleanup;
  }
...
cleanup:
  ...
  if (pixels)
    free(pixels);
  if (err > 0)
    return luaL_error(L, errstr(err));
...
static int l_decode_paletted_png(lua_State *L)
{
  ...
  if (!lua_istable(L, 1))
  {
    err = ERR_DECPLT_PAR1NOTBL;
    goto cleanup;
  }
  if (!lua_isstring(L, 2))
  {
    err = ERR_DECPLT_PAR2NOSTR;
    goto cleanup;
  }
