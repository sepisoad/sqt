...
local DATA = require('libs.lua.app.data')
...
---@param data any
---@param path string
local function save_item_data_to_file(file, header, data, path)
  log.dbg("saving .WAD data into file")

  if header.Type == WadItemType.Lumpy then
    print("@@@@@@@@@@@@@@@@@@@@@@@@@@")
    local w, pos = string.unpack("=i", data, 1)
    local h = string.unpack("=i", data, pos)
    print(w)
    print(h)
    print("@@@@@@@@@@@@@@@@@@@@@@@@@@")
    ---@type LumpHeader
    local lmp_header = {
      Width = string.unpack("=i", data),
      Height = string.unpack("=i", data, 5),
      Data = string.unpack(data, 9)
    }

    local qoi_data, err = qoi.encode_indexed(
      lmp_header.Data,
      DATA.quake_palette,
      lmp_header.Width,
      lmp_header.Height)
    if err then
      log.fatal(string.format("failed to decode '%s' using default palette", path), err)
    end
    if not write.all(file, qoi_data) then
      print("-----------------------------------")
